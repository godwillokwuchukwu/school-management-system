        ...
        for att in AttendanceRecord.objects.all():
            ...
            if dim_student and dim_date:
                FactAttendance.objects.update_or_create(
                    student=dim_student, date=dim_date, defaults={"status": att.status}
                )

        for grade in Grade.objects.select_related(
            "enrollment__student", "enrollment__school_class"
        ).all():
            dim_student = DimStudent.objects.filter(
                student_id=grade.enrollment.student.id
            ).first()
            dim_subject = DimSubject.objects.filter(subject_id=grade.subject_id).first()
            dim_class = DimClass.objects.filter(
                class_id=grade.enrollment.school_class.id
            ).first()
            ...
            if dim_student and dim_date:
                ...
                FactPayment.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        or (fee.status == "pending" and fee.due_date < today),
                    },
                )
