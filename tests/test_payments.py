...
def test_base_payment_gateway_not_implemented():
    ...
    with pytest.raises(NotImplementedError):
        gateway.initialize_payment("test@example.com", 100.0, "REF123", "http://callback")
    with pytest.raises(NotImplementedError):
...
def test_dummy_payment_gateway():
    ...
    res = gateway.initialize_payment("test@example.com", 50000.0, "REF-TEST-001", "https://example.com/callback")
    ...
    res_auto = gateway.initialize_payment("test@example.com", 50000.0, "", "https://example.com/callback")
...
def test_paystack_payment_gateway(monkeypatch):
    ...
    res = gateway.initialize_payment("test@example.com", 1000.0, "PSTK-01", "http://callback")
    assert res["status"] is True
    ...
    res2 = gateway_with_key.initialize_payment("test@example.com", 1000.0, "PSTK-02", "http://callback")
...
def test_flutterwave_payment_gateway(monkeypatch):
    ...
    res = gateway.initialize_payment("test@example.com", 2000.0, "FLW-01", "http://callback")
    assert res["status"] is True
